...
namespace ovr {
	...
	//NOTE: orientation and position(translation) calc here
...
public:
	...
	RiftApp() {
		...
		ovr::for_each_eye([&](ovrEyeType eye) {
			...
			//cse190: Adjust eye separation here 
			//increment the HMDTOEYEoffset
...
protected:
	...
	void draw() final override {
		...
		if (iod_up) {
			...
			if (_viewScaleDesc.HmdToEyeOffset[0].x < -2.0f) {
				_viewScaleDesc.HmdToEyeOffset[0].x = -2.0f;
			}
			if (_viewScaleDesc.HmdToEyeOffset[0].x > 2.0f) {
				_viewScaleDesc.HmdToEyeOffset[0].x = 2.0f;
			}
			if (_viewScaleDesc.HmdToEyeOffset[1].x < -2.0f) {
				_viewScaleDesc.HmdToEyeOffset[1].x = -2.0f;
			}
			if (_viewScaleDesc.HmdToEyeOffset[1].x > 2.0f) {
				_viewScaleDesc.HmdToEyeOffset[1].x = 2.0f;
			}

			float offset = _viewScaleDesc.HmdToEyeOffset[0].x *  0.01f;
			float offsetR = _viewScaleDesc.HmdToEyeOffset[1].x * 0.01f;

			//moving apart 
			_viewScaleDesc.HmdToEyeOffset[0].x += offset;
			_viewScaleDesc.HmdToEyeOffset[1].x += offsetR;
		...
		//reset
		if (right_click) {
			_viewScaleDesc.HmdToEyeOffset[0] = reset;
			_viewScaleDesc.HmdToEyeOffset[1] = Right_reset;
			//	right_click = false; 
		}
	...
	virtual void renderScene(const glm::mat4 & projection, const glm::mat4 & headPose, int i) = 0;
};
...
protected:
	...
	void initGl() override {
		RiftApp::initGl();
		glClearColor(0.2f, 0.1f, 0.7f, 0.0f);
	...
	void renderScene(const glm::mat4 & projection, const glm::mat4 & headPose, int i) override {
		...
		if (scale_down) {
			...
			Window::idle_callback(1.005f);
		}
		//x_axis
		else if (x_down) {
			Window::transCube(-0.05f, 0); 
		}
		else if (x_up) {
			Window::transCube(0.05f, 0); 
		}
		...
		if (Xcounter == 1) {
			...
			Window::V = ovr::toGlm(HandPoses[ovrHand_Left]);
			//Window::V[3] =  glm::vec4(HandPoses[ovrHand_Left].Position.x, HandPoses[ovrHand_Left].Position.y, HandPoses[ovrHand_Left].Position.z, 1.0f);
		}
